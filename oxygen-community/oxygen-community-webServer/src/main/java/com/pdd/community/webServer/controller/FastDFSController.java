...
public class FastDFSController {
	 ...
	 */
	@PostMapping("/upload")
	public MsgReturn<Object> upload(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (resolver.isMultipart(request)) {
				...
				if (fileMap == null || fileMap.size() == 0) {
					...
					LOG.info("上传文件为空！");
				} else if (fileMap.size() > 10) {
					LOG.info("上传文件数量超过 10了！");
				} else {
					...
					while (entrySetIterator.hasNext()) {
						...
						if(1024 * 1024 * 100 < entry.getValue().getSize()){
							LOG.info("图片 " + entry.getValue().getOriginalFilename() + " 大小超过限制 (100MB)，上传操作终止。");
						}
	...
	public String download() {
		InputStream downloadFile = FastDFSClient.downloadFile("group1/M00/00/00/wKgB-1sz26CAaPY7AASR1Jv_l9c525.png");
	...
	public String delete() {
		int result = FastDFSClient.deleteFile("group1/M00/00/00/wKgB-1sz26CAaPY7AASR1Jv_l9c525.png");
		if(result == 0) {
			LOG.info("--->删除成功！");
			return "success";
		}else {
