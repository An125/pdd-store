...
public class OrderReturnCancelController {
	@Autowired
	private StoreOrderFormService storeOrderFormService;
	@PostMapping("/orderReturnCancel")
	public MsgReturn<OrderFormVo> orderReturnCancel(OrderFormParams orderFormParams) throws Exception {
		// 1.创建新的返回对象（这里泛型指返回消息中带的对象是订单Vo）
		MsgReturn<OrderFormVo> msg = new MsgReturn<OrderFormVo>();
		
		
		//	2.处理业务逻辑
			int result = storeOrderFormService.orderReturnCancel(orderFormParams);
		// 3.把对象设置进返回消息中
			if(result>0) {
				msg.success("取消退货成功！");
			}else {
