...
public class FastDFSController {
	 ...
	 */
	@PostMapping("/upload")
	public MsgReturn<Object> upload(UserParams userParams,HttpServletRequest request,HttpServletResponse response) throws Exception {
			...
			if (resolver.isMultipart(request)) {
				...
				if (fileMap == null || fileMap.size() == 0) {
					...
					while (entrySetIterator.hasNext()) {
						...
						//存到数据库中
						UserVo vo = storeUserService.pictureUpload(userParams.getMobile(), fastDFS_Id);
						if(vo==null) {
							msg.failed("文件存储失败！");
						}else {
							msg.setReturnObj(vo.getTxpath());
							msg.success("文件存储成功！");
						}
					}
				}
