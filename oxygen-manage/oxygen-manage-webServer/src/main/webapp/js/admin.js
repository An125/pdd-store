...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('oxygen-upms-systemid', systemid);
		$.cookie('oxygen-upms-systemname', systemname);
		$.cookie('oxygen-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('oxygen-upms-systemid') || 1;
	var systemname = $.cookie('oxygen-upms-systemname') || 'oxygen-upms-server';
	var systemtitle = $.cookie('oxygen-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
